...
func createCard(title, subTitle, dateStr, wetonStr string, statusType int, diffDays int) fyne.CanvasObject {
	...
	// Jika statusType == 0 atau tidak dikenal, badge tidak muncul (transparan)
...
func main() {
	...
	lblSelectedWetonDate := widget.NewLabel("")
	...
	btnOpenWeton.Icon = theme.AccountIcon() // Ikon user untuk kelahiran

	btnOpenWeton.OnTapped = func() {
		// Menggunakan fungsi date picker yang sama (REUSE)
		createCalendarPopup(myWindow.Canvas(), wetonDate,
			func(realtimeDate time.Time) {
				wetonDate = realtimeDate
				updateWetonDateLabel(wetonDate)
			},
			func(finalDate time.Time) {
				wetonDate = finalDate
	...
	// Struktur: Header -> Tabs -> Footer
	mainContent := container.NewBorder(
		headerContainer, // Top
		container.NewPadded(footerSection), // Bottom
		nil, nil,
		container.NewPadded(tabs), // Center (Tabs)
	)
